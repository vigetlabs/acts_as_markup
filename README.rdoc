...
+:markdown_options+, +:textile_options+ or +:wikitext_options+. RDoc does not
support any useful options. The options should be given as an array of
arguments. You can specify options for multiple languages when allowing more
than one. See each library's documentation for more details on what options
are available.
